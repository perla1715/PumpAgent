...
def main() -> None:
    ...
    try:
        ...
        if not config.diagnostic_symbols:
            print_scanner_configuration(config)
...
def print_scanner_configuration(config: Config) -> None:
    print("SCANNER CONFIGURATION")
    print(f"OHLCV lookback: {config.ohlcv_lookback_minutes} minutes")
    print(f"OI lookback: {config.oi_lookback_minutes} minutes")
    print(f"Closed candles only: {USE_CLOSED_CANDLES_ONLY}")
    print(f"Timestamp alignment required: {OHLCV_OI_ALIGNMENT_REQUIRED}")
    print(f"Volume baseline: {VOLUME_BASELINE_CANDLES} candles")
